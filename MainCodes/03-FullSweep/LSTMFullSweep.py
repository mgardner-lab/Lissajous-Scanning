"""1D LSTM full hyperparameter sweep on Lissajous-scanned MNIST.

Sweeps every combination of (hidden_size, num_layers, dropout, lr) at each
sample size N, using the per-N Lissajous winner (fx, fy, fs). The Lissajous
signal is fed to the LSTM as a sequence of length N (one feature per step);
classification uses the final hidden state through a small FC head.

Builds the Lissajous datasets on first use (one pickle per N), trains every
config, and records accuracy + precision + recall + F1 + 10k inference time
...
Two parallel instances on the same GPU (PowerShell):
    # one-time, build datasets to avoid race condition:
    $env:LSTM_BUILD_ONLY="1"; python LSTMFullSweep.py
    # then in two separate terminals:
    $env:LSTM_SHARD_INDEX="0"; $env:LSTM_SHARD_COUNT="2"; python LSTMFullSweep.py
    $env:LSTM_SHARD_INDEX="1"; $env:LSTM_SHARD_COUNT="2"; python LSTMFullSweep.py

Output: results_lstm/<name>/{metrics.csv, final_metrics.json}    per config
        results_lstm/summary.csv                                  aggregated
"""
...
NUM_LAYERS = (1, 2, 3, 4)
DROPOUTS = (0.0, 0.2)
DIRECTIONS = (False, True)              # False = unidirectional, True = bidirectional
...
class LSTMClassifier(nn.Module):
    """LSTM over the 1D signal; final hidden state(s) -> FC -> classes.

    Supports unidirectional and bidirectional LSTMs. For bidirectional, the
    classifier sees the concatenated final hidden states of both directions
    (so the FC input size doubles).
    """
    def __init__(self, num_classes=10, hidden_size=128, num_layers=1,
    ...
    def forward(self, x):
        # x: (batch, 1, N) -> (batch, N, 1)
        x = x.transpose(1, 2)
        _, (h, _) = self.lstm(x)
        if self.bidirectional:
            # h: (num_layers * 2, batch, hidden_size). Last layer = two directions.
...
def train_one(N, fx, fy, fs, hidden, layers, drop, bidir, lr, seed, run_dir):
    ...
    model = LSTMClassifier(NUM_CLASSES, hidden, layers, drop,
    ...
    final = {
        ...
        "hidden_size": hidden, "num_layers": layers, "dropout": drop,
...
# === Sweep orchestration ===
def exp_name(N, fx, fy, fs, hidden, layers, drop, bidir, lr, seed):
    direction = "bi" if bidir else "uni"
    return (f"N{N}_fx{fx}_fy{fy}_fs{fs}_h{hidden}_L{layers}"
            f"_drop{drop}_{direction}_lr{lr}_seed{seed}").replace(".", "p")


def write_summary():
    rows = []
    for d in sorted(RESULTS_DIR.iterdir()) if RESULTS_DIR.exists() else []:
        if not d.name.startswith("N"):
            continue
...
if __name__ == "__main__":
    ...
    build_only = os.environ.get("LSTM_BUILD_ONLY") == "1"
    shard_idx = int(os.environ.get("LSTM_SHARD_INDEX", "0"))
    shard_cnt = int(os.environ.get("LSTM_SHARD_COUNT", "1"))
    if shard_idx < 0 or shard_idx >= shard_cnt:
        raise SystemExit(f"LSTM_SHARD_INDEX={shard_idx} out of range for COUNT={shard_cnt}")

    if build_only:
        print("BUILD-ONLY mode: pre-building datasets so parallel shards don't race.")
        for N in SAMPLE_SIZES:
            fx, fy, fs = LISSAJOUS_PER_N[N]
            print(f"  Ensuring dataset N={N} (fx={fx}, fy={fy}, fs={fs})")
            ensure_dataset(N, fx, fy, fs)
        print("Datasets ready. Re-run without LSTM_BUILD_ONLY to start training.")
        raise SystemExit(0)

    jobs = [(N, *LISSAJOUS_PER_N[N], hidden, layers, drop, bidir, lr, seed)
            for N in SAMPLE_SIZES for hidden in HIDDEN_SIZES
            for layers in NUM_LAYERS for drop in DROPOUTS
            for bidir in DIRECTIONS for lr in LEARNING_RATES for seed in SEEDS]

    my_jobs = [(i, j) for i, j in enumerate(jobs, 1) if (i - 1) % shard_cnt == shard_idx]
    tag = f"shard {shard_idx}/{shard_cnt}" if shard_cnt > 1 else "single instance"
    print(f"Device: {DEVICE}  |  total jobs: {len(jobs)}  |  this shard: {len(my_jobs)}  ({tag})")

    t_sweep = time.perf_counter()
    ok = skipped = failed = 0
    for i, job in my_jobs:
        name = exp_name(*job)
    ...
    print(f"\nDone in {(time.perf_counter()-t_sweep)/60:.1f} min  |  "
          f"{ok} ok, {skipped} skipped, {failed} failed  ({tag})")
    if shard_cnt == 1 or shard_idx == 0:
        write_summary()
