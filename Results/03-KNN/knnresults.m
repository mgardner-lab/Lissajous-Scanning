%% KNN Parametric Analysis
clear; close all; clc;

T = readtable("knn_results.csv");

outDir = "KNN_parametric_outputs";
if ~exist(outDir, "dir"); mkdir(outDir); end

predictors = {'N','fx','fy','fs','n_neighbors','metric','weights'};
metrics = {'accuracy','inference_10k_s'};
summaryMetrics = {'accuracy','precision','recall','f1','inference_10k_s','fit_time_s'};

%% Summary table by sample number
summaryByN = groupsummary(T, "N", {'mean','std','min','max'}, summaryMetrics);
writetable(summaryByN, fullfile(outDir, "KNN_summary_by_N.xlsx"));
...
writetable(allSummaries, fullfile(outDir, "KNN_summary_by_parameter.xlsx"));
...
writetable(statsOverall, fullfile(outDir, "KNN_overall_stats.xlsx"));
writetable(tukeyAll, fullfile(outDir, "KNN_tukey_posthoc.xlsx"));
...
writetable(statsByN, fullfile(outDir, "KNN_stats_by_N.xlsx"));
writetable(tukeyByN, fullfile(outDir, "KNN_tukey_by_N.xlsx"));

%% Paired comparisons for binary parameters
binaryPredictors = {'metric','weights'};
pairedResults = table();
...
writetable(pairedResults, fullfile(outDir, "KNN_paired_binary_tests.xlsx"));
...
writetable(topAcc, fullfile(outDir, "KNN_top_models_by_accuracy.xlsx"));
writetable(topTime, fullfile(outDir, "KNN_top_models_by_time.xlsx"));

%% Plots: sample number
plotBox(T, "N", "accuracy", "Accuracy (%)", ...
    fullfile(outDir, "knn_acc_v_N.png"));

plotBox(T, "N", "inference_10k_s", "10k Inference Time (s)", ...
    fullfile(outDir, "knn_time_v_N.png"));

%% Plots: each KNN parameter split by N
paramsNoN = {'fx','fy','fs','n_neighbors','metric','weights'};

for p = 1:numel(paramsNoN)
    pred = paramsNoN{p};

    plotMeanSDByN(T, pred, "accuracy", "Accuracy (%)", ...
        fullfile(outDir, "knn_acc_v_" + pred + ".png"));

    plotMeanSDByN(T, pred, "inference_10k_s", "10k Inference Time (s)", ...
        fullfile(outDir, "knn_time_v_" + pred + ".png"));
end
...
function R = pairedBinaryTest(T, pred, metric)
    ...
    otherVars = {'N','fx','fy','fs','n_neighbors','metric','weights'};
