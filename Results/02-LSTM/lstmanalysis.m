%% LSTM Parametric Analysis
clear; close all; clc;

T = readtable("LSTMFullSweep.csv");

outDir = "LSTM_parametric_outputs";
if ~exist(outDir, "dir"); mkdir(outDir); end

predictors = {'N','fx','fy','fs','direction','dropout','hidden_size','lr','num_layers'};
metrics = {'accuracy','inference_10k_s'};
metricNames = {'Accuracy (%)','10k Inference Time (s)'};

%% Summary table by sample number
summaryByN = groupsummary(T, "N", {'mean','std','min','max'}, ...
    {'accuracy','precision','recall','f1','inference_10k_s','train_time_s'});
writetable(summaryByN, fullfile(outDir, "LSTM_summary_by_N.xlsx"));
...
for p = 1:numel(predictors)
    ...
    G = groupsummary(T, pred, {'mean','std','min','max'}, ...
        {'accuracy','precision','recall','f1','inference_10k_s','train_time_s'});
...
writetable(allSummaries, fullfile(outDir, "LSTM_summary_by_parameter.xlsx"));

%% Overall one-way ANOVA, Tukey-Kramer, and Spearman correlation
statsOverall = table();
tukeyAll = table();
...
writetable(statsOverall, fullfile(outDir, "LSTM_overall_stats.xlsx"));
writetable(tukeyAll, fullfile(outDir, "LSTM_tukey_posthoc.xlsx"));
...
writetable(statsByN, fullfile(outDir, "LSTM_stats_by_N.xlsx"));
writetable(tukeyByN, fullfile(outDir, "LSTM_tukey_by_N.xlsx"));

%% Paired comparisons for binary parameters
binaryPredictors = {'direction','dropout','hidden_size','lr'};
pairedResults = table();
...
writetable(pairedResults, fullfile(outDir, "LSTM_paired_binary_tests.xlsx"));
...
for i = 1:numel(Ns)
    ...
    [~, ia] = max(Ti.accuracy);
    [~, it] = min(Ti.inference_10k_s);

    topAcc = [topAcc; Ti(ia,:)];
    topTime = [topTime; Ti(it,:)];
end

writetable(topAcc, fullfile(outDir, "LSTM_top_models_by_accuracy.xlsx"));
writetable(topTime, fullfile(outDir, "LSTM_top_models_by_time.xlsx"));

%% Plots: sample number
plotBox(T, "N", "accuracy", "Accuracy (%)", ...
    fullfile(outDir, "lstm_acc_v_N.png"));

plotBox(T, "N", "inference_10k_s", "10k Inference Time (s)", ...
    fullfile(outDir, "lstm_time_v_N.png"));

%% Plots: each parameter split by N
paramsNoN = {'fx','fy','fs','direction','dropout','hidden_size','lr','num_layers'};

for p = 1:numel(paramsNoN)
    pred = paramsNoN{p};

    plotMeanSDByN(T, pred, "accuracy", "Accuracy (%)", ...
        fullfile(outDir, "lstm_acc_v_" + pred + ".png"));

    plotMeanSDByN(T, pred, "inference_10k_s", "10k Inference Time (s)", ...
        fullfile(outDir, "lstm_time_v_" + pred + ".png"));
end
...
function R = pairedBinaryTest(T, pred, metric)
    ...
    otherVars = {'N','fx','fy','fs','direction','dropout','hidden_size','lr','num_layers'};
